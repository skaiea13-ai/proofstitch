...
The script refuses to reuse an existing `proofstitch` service, cleanup queue, or cleanup service account. It selects Gemini Developer API mode explicitly, deploys the service disabled, grants a fresh cleanup identity delete authority only on that service, checks service-local IAM, and uses Cloud Asset IAM Policy Analyzer to reject `allUsers` or `allAuthenticatedUsers` access inherited from the full project hierarchy. Newly created resources can take a few seconds to appear in Cloud Asset Inventory and Cloud Run routing, so incomplete analyses and transitional anonymous responses are retried for the same bounded IAM propagation window and still fail closed. Anonymous probes target the application root and must ultimately return only `401` or `403`.
